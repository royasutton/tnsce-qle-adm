...
| `list-ports` | `lp` | FC ports. Column order: `[idx] wwn (label)  isp:  host:  status:  topology:  config:`. All field names in cyan. `status:` values: `Online`=green, `Diagnostics`=yellow, `Unknown`=dim, all others (Linkdown, Offline, Bypassed, Not Present)=red. `topology:` values: `P2P` `Loop` `FPort` `NPort` `LPort` `NPIV`=plain, `Unknown`=dim. `config:` values: `managed`=green, `unmanaged`=yellow. |
