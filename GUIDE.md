...
`qle_adm.sh` v2.0: QLogic FC Target Manager for TrueNAS SCALE CE
...
│
└── qle_adm.sh
    ├── config.json  (/mnt/<pool>/)   ← single source of truth
    ├── /etc/scst.conf                ← rebuilt at boot and on sync
    ├── /etc/modprobe.d/              ← module params (regenerated by sync)
    └── /etc/systemd/system/          ← boot service (regenerated by sync)
```
...
Files in `/etc` are regenerated by `sync` after any BE change or upgrade.

### After an upgrade or boot environment change

```bash
/mnt/<pool>/admin/qle_adm/qle_adm.sh sync
systemctl restart scst
/mnt/<pool>/admin/qle_adm/qle_adm.sh status
```

`sync` restores `/etc` files and rebuilds scst.conf from config.json.
SCST must be restarted to read the updated scst.conf after an upgrade.
...
only needed after a WUI iSCSI save wiped the scst.conf FC block, or after
an upgrade or BE change wiped `/etc` files. Running `sync` at any other time
is harmless — it rebuilds the files from config.json without touching sysfs.
