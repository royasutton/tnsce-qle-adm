...
| `list-extents` | `le` | SCST extents with size, config state (`cfg:[open]`/`cfg:[in group]`/`cfg:[unmapped]`), and live sysfs state (`sysfs:[no sysfs]`/`sysfs:[mapped]`/`sysfs:[connected]`/`sysfs:[active]`). The two columns are sourced independently: `cfg:` reflects what `config.json` says; `sysfs:` reflects the running SCST kernel. `cfg:[unmapped] sysfs:[mapped]` means a LUN mapping exists in sysfs that config.json has no record of — run `sync` to reconcile. `sysfs:[no sysfs]` = config exists but not yet applied to running SCST, run `sync --apply`. `sysfs:[mapped]` = in sysfs, no initiator session. `sysfs:[connected]` = session present, no I/O yet. `sysfs:[active]` = session present with I/O, shows active commands and session R/W totals. |
