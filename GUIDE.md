...
Files in `/etc` are regenerated by `sync --system` after any BE change or upgrade.

### After an upgrade or boot environment change

```bash
/mnt/<pool>/admin/qle_adm/qle_adm.sh sync --system --restart
/mnt/<pool>/admin/qle_adm/qle_adm.sh status
```

`sync --system` restores `/etc` files and rebuilds scst.conf from config.json.
`--restart` restarts SCST so it reads the updated scst.conf in one step.
...
| `/etc/modprobe.d/` | ✓ | ✗ | ✗ | `sync` |
| `/etc/systemd/system/` | ✓ | ✗ | ✗ | `sync --system` |
...
- After an upgrade or BE change wiped `/etc` files: `sync --system`
